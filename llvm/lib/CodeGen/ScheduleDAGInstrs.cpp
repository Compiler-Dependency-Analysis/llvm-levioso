...
void ScheduleDAGInstrs::initSUnits() {
  ...
  single_mode = true; // indicates that instructions within the region are in a single MBB or not
  auto B1 = RegionBegin->getParent();
  auto tempRegion = RegionEnd;
  tempRegion--;
  auto B2 = tempRegion->getParent();
  ...
  {
    MachineFunction &MF = *RegionBegin->getParent()->getParent();
