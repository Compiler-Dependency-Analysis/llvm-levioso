...
protected:
  //Ali: this function is updated and supports the distinctions between pre- and post-RA scheduling
  void scheduleRegions(ScheduleDAGInstrs &Scheduler, bool FixKillFlags, bool preRA);
};
...
bool MachineScheduler::runOnMachineFunction(MachineFunction &mf) {
  ...
  scheduleRegions(*Scheduler, false, true);
...
bool PostMachineScheduler::runOnMachineFunction(MachineFunction &mf) {
  ...
  scheduleRegions(*Scheduler, true, false);
...
void MachineSchedulerBase::scheduleRegions(ScheduleDAGInstrs &Scheduler,
  ...
  else
  {
    ...
    {
      if (RegionBegin == nullptr && MBB->size() != 0)
        RegionBegin = MBB->begin();

      if (MBB->size() == 0)
        continue;

      for (RegionEnd = MBB->begin(); RegionEnd != MBB->end(); RegionEnd++)
      {
        instNum++;
      }
    }

    MBBRegions.push_back(SchedRegion(RegionBegin, RegionEnd, instNum));
    ...
    for (MBBRegionsVector::iterator R = MBBRegions.begin();
      ...
      // Notify the scheduler of the region, even if we may skip scheduling
      // it. Perhaps it still needs to be bundled.
      Scheduler.enterRegion(&*MF->begin(), I, RegionEnd, NumRegionInstrs);
...
// Ali: this function is going to find the dependencies between instructions within the region
void ScheduleDAGMI::findDep()
{
  LLVM_DEBUG(dbgs() << "Finding dependencies between instructions of a Function.\n");

  // errs() << "[MachineScheduler::findDep] Finding dependencies between instructions of a Function.\n";
  ...
  for (const SUnit &SU : SUnits)
  {
    ...
    // skip if SUnit do not point to a machine instructions
    if (!SU.isInstr())
      continue;

    for (const SDep &SD : SU.Succs)
    {
      if (SD.getSUnit()->isInstr())
      {
        MachineInstr *sdi = &(*SD.getSUnit()->getInstr());
        mi->data_dependent.insert(sdi);
      }
    }
  }
}
